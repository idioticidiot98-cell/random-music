...
RunService.RenderStepped:Connect(function()
	...
	local HdPos = Head.Position
	local TrsoLV = Torso.CFrame.LookVector

	local Dist = (HdPos - CamCF.Position).Magnitude
	local Diff = HdPos.Y - CamCF.Position.Y

	if Dist > 0 then

		local Vertical = -(math.asin(Diff/Dist) * HeadVertFactor)

		local Direction = (HdPos - CamCF.Position).Unit
		local Horizontal = -((Direction:Cross(TrsoLV)).Y * HeadHorFactor)

		local Target = Neck.C0 * CFrame.Angles(Vertical,0,Horizontal)

		Neck.C0 = Neck.C0:Lerp(Target, SmoothSpeed)
...
if Hat then
Global.AlignPart(Hat.Handle,GUN.Baseplate,Vector3.new(0.1, 0, 0),Vector3.new(0, 180 ,0))
end
local Hat = game.Players.LocalPlayer.Character:FindFirstChild('Accessory (AlligatorJaw)')
if Hat then
Global.AlignPart(Hat.Handle,GUN.Hole,Vector3.new(-1.3, 0.1, 0),Vector3.new(0, 180 ,0))
end
local Hat = game.Players.LocalPlayer.Character:FindFirstChild('Accessory (Player Shadow)')
if Hat then
Global.AlignPart(Hat.Handle,Vis.Weld.Part0,Vector3.new(0, 3, 0),Vector3.new(0, 0 ,0))
end
local Hat = game.Players.LocalPlayer.Character:FindFirstChild('Accessory (Player Shadow)1')
if Hat then
Global.AlignPart(Hat.Handle,Vis.Weld.Part0,Vector3.new(0, 3, 0),Vector3.new(0, 90 ,0))
end
...
coroutine.resume(coroutine.create(function()
	while true do
		...
		if not alreadyfixing and NAMEMODE:IsDescendantOf(game) then
			NAMEMODE.TextColor3 = Gunneon.Color
...
function SingularityBeam()
	...
	local chargeball = CreatePart2(3,Effects,"Neon",0,0,shade,"Charge Ball",Vector3.new(.5,.5,.5),false)
	...
	local killbeam = CreatePart2(3,Effects,"Neon",0,0,shade,"Kill Beam",Vector3.new(2.5,KillDist,2.5))
	MakeForm(killbeam,"Cyl")
	killbeam.Touched:Connect(function(victim)
		if voidy == false and banishing == false and killa ~= "Nil" then
			mdmg(victim.Position,5)
		elseif voidy == true and banishing == false and killa ~= "Nil" then
			voidlc(victim.Position,5)
		elseif banishing == true and voidy == false and killa ~= "Nil" then
			local HIT,POS = CastProperRay(Hole.Position, Mouse.Hit.p, 1000, Character)
			if HIT ~= nil then
				if HIT.Parent ~= game:GetService("Workspace") and HIT.Parent.ClassName ~= "Folder" then
					Banish(HIT.Parent)
				end
			end
		elseif voidy == false and banishing == false and killa == "Nil" then
	...
	coroutine.resume(coroutine.create(function()
		...
		while killbeam:IsDescendantOf(game) do
			...
			if isdoingathing then
				coroutine.resume(coroutine.create(function()
					Lightning(Holetwo.Position,Mouse.Hit.p,15,3.5,Gunneon.Color,25,0,1,0,true,55)
				end))
...
function QuickLcKill()
	...
	for i,a in pairs(CRMoment) do
		if a:IsA("Folder") and a.Name ~= "DefaultChatSystemChatEvents" or a:IsA("Model") or a:IsA("Script") or a:IsA("LocalScript") or a:IsA("WorldModel") or a:IsA("ModuleScript") then
			a:Destroy()
		end
	end
...
function destroycannons()
	coroutine.resume(coroutine.create(function()
		attack = true
		walkspeed = 0
		local ree = CreateSound(139989857274709,Head,10,1,false)
...
Mouse.KeyDown:Connect(function(k)
	...
	elseif k == "5" and taunt.Value ~= "iNSaNiTY" and nomain == false then
		...
		TauntRemote:Fire("iNSaNiTY",1842728390)
	...
	elseif k == "u" and nomain == false then
		if taunt.Value ~= "Sit" then
			TauntRemote:Fire("Sit",1843295825)
			walkspeed = 10
		else
			TauntRemote:Fire("None",1841425036)
			walkspeed = 10
	...
	elseif k == "t" and nomain == false then
		if taunt.Value ~= "Fave" then
			TauntRemote:Fire("Fave",9042666419)
			walkspeed = 10
		else
	...
	elseif k == "y" and nomain == false then
		if taunt.Value ~= "Krump" then
			TauntRemote:Fire("Krump",99744372463911)
			walkspeed = 50
		else
			TauntRemote:Fire("None",1841425036)
			walkspeed = 10
...
Work.ChildAdded:Connect(function(v)
	if stopeverything then task.wait(math.huge) end
	if v:IsA("Message") and v.Text ~= "I kinda need you to have a game.Players.LocalPlayer.Character.HumanoidRootPart to be able to do anthing,try again once you have one lol" then
		v.Text = ""
...
GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
		...
		if Torsovelocity < sensitivity and taunt.Value == "None" then
			...
			if attack == false then
				...
				if GUN and Outlines then
					...
					if Gunneon then
						Gunneon.Transparency = 0
					end
					if OutBase then
						OutBase.Transparency = 0
					end
				end
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "KingMan" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0,0,0 + .085 * math.cos(Sine / 12)) * CFrame.Angles(math.rad(-4 + 2 * math.sin(sine / 12)),math.rad(0),math.rad(0)) * CFrame.Angles(math.rad(0 + 2 * -math.cos(sine / 12)),math.rad(0),math.rad(0)),1 / 3)
					Neck.C0 = Clerp(Neck.C0,nc0 * CFrame.new() * CFrame.Angles(math.rad(0 - 10 * math.sin(Sine / 74)),math.rad(0),math.rad(0)) * CFrame.Angles(math.rad(0 + 10 * math.cos(Sine / 74)),math.rad(0),math.rad(0)),1 / 3)
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "Lost" then
			...
			if attack == false then
				...
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-25,-.15) * CFrame.Angles(math.rad(90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Lost" then
			Anim = "Walk"
			if attack == false then
				...
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-25,-.15) * CFrame.Angles(math.rad(-115),math.rad(0),math.rad(0)),1/2)
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "Toxic" then
			...
			if attack == false then
				...
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,35,-.15) * CFrame.Angles(math.rad(90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Toxic" then
			Anim = "Walk"
			if attack == false then
				...
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,35,-.15) * CFrame.Angles(math.rad(-115),math.rad(0),math.rad(0)),1/2)
		...
		elseif Torsovelocity >= sensitivity and taunt.Value == "Berger" then
			...
			if attack == false then
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0,0,0 + 0.05 * math.cos(sine / 12)) * CFrame.Angles(math.rad(20),math.rad(0 + 2.5 * math.cos(sine / 12)),math.rad(0 - 2.5 * math.cos(sine / 12))),1 / 3)
					...
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -1 - 0.05 * math.cos(sine / 12), -0.01) * CFrame.Angles(math.rad(35 * math.cos(sine / 12)) + math.rad(0 + 3 * math.sin(sine / 12)), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-0), math.rad(0), math.rad(0)), 0.7 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -1 - 0.05 * math.cos(sine / 12), -0.01) * CFrame.Angles(math.rad(-35 * math.cos(sine / 12)) + math.rad(0 + 3 * math.sin(sine / 12)), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(-0), math.rad(0), math.rad(0)), 0.7 / 3)
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "Fallen" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0 + .15 * math.sin(sine / 24), 0, 3 + 0.25 * math.cos(sine / 24)) * CFrame.new(0 + -.15 * -math.sin(sine / 24), 0, 0 + -0.25 * -math.sin(sine / 24)) * CFrame.Angles(math.rad(-5 + 5 * math.cos(sine / 24)) + math.rad(0 + 5 * math.sin(sine / 24)), math.rad(-0 + 3 * math.sin(sine / 24)) + math.rad(0 + -3 * -math.sin(sine / 24)), math.rad(0 + 3 * math.sin(sine / 24))), 0.7 / 3)
					Neck.C0 = Clerp(Neck.C0, nc0 * CFrame.new(0, 0, 0 + ((1) - 1)) * CFrame.Angles(math.rad(0), math.rad(-0 + -5 * -math.cos(sine / 24)) + math.rad(0 + 5 * math.sin(sine / 24)), math.rad(7 * math.sin(sine / 24))), 0.7 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0, CFrame.new(.8, 0.35 + 0.05 * math.cos(sine / 24), -.5) * CFrame.Angles(math.rad(0), math.rad(0 + -3 * math.cos(sine / 24)) + math.rad(0 + -3 * math.sin(sine / 24)), math.rad(-90 + -3 * math.cos(sine / 24)) + math.rad(0 + 3 * math.sin(sine / 24))) * rscp, 0.7 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0, CFrame.new(-1.5, 0.5 + 0.05 * math.cos(sine / 24), 0) * CFrame.Angles(math.rad(0), math.rad(10 + 5 * math.cos(sine / 24)) + math.rad(0 + 5 * math.sin(sine / 24)), math.rad(-12 + -5 * -math.cos(sine / 24)) + math.rad(0 + -10 * math.sin(sine / 24))) * lscp, 0.7 / 3)
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -1 - 0.05 * math.cos(sine / 24), -0.01) * CFrame.Angles(math.rad(0), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-0 + -7 * math.cos(sine / 24)), math.rad(-15 + -3 * math.sin(sine / 24)), math.rad(0)), 0.7 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -1 - 0.05 * math.cos(sine / 24), -0.01) * CFrame.Angles(math.rad(0), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(0 + 7 * math.cos(sine / 24)), math.rad(12 + 3 * math.sin(sine / 24)), math.rad(0)), 0.7 / 3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Fallen" then
			Anim = "Walk"
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "iNSaNiTY" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0,0,3 - .5 * math.sin(sine/50)) * CFrame.Angles(math.rad(10 + 3 * math.sin(sine/50)),math.rad(-8 + 3 * math.sin(sine/50)),math.rad(-1.4)) * CFrame.Angles(math.rad(0 + -5 * math.cos(sine / 50)),math.rad(0),math.rad(0)),.7/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.Angles(math.rad(19 + 9 * math.sin(sine / 74)),math.rad(6.3 + 4 * math.sin(sine / 50)),math.rad(-1.8 + -2 * math.sin(sine / 50))) * CFrame.Angles(math.rad(0),math.rad(0 + -4 * math.cos(sine / 50)),math.rad(-0 + 3 * math.cos(sine / 50))),1)
				...
				RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.5,0) * CFrame.Angles(math.rad(-59.2-4*math.sin(sine/50)),math.rad(-36.3),math.rad(-18.4)) * RIGHTSHOULDERC0,.7/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5,0) * CFrame.Angles(math.rad(5.6 - 4*math.sin(sine/50)),math.rad(3 - 4*math.sin(sine/50)),math.rad(-3 - 4*math.sin(sine/50))) * CFrame.Angles(math.rad(0 + 6 * math.cos(sine / 50)),math.rad(0 + 6 * math.cos(sine / 50)),math.rad(-0 + 6 * math.cos(sine / 50))) * LEFTSHOULDERC0,.7/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1,-.1) * CFrame.Angles(math.rad(-9.9 + 4*math.sin(sine/50)),math.rad(-8.1 + 4 *math.sin(sine/50)),math.rad(1.4)) * CFrame.Angles(math.rad(0 + -7 * math.cos(sine / 50)),math.rad(90),math.rad(0)),1/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-.8,-.51 - .03125 * math.sin(sine/50)) * CFrame.Angles(math.rad(-32 + 4 *math.sin(sine/50)),math.rad(8 + 4 *math.sin(sine/50)),math.rad(1.6)) * CFrame.Angles(math.rad(0 + -7 * math.cos(sine / 50)),math.rad(-90),math.rad(0)),1/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity < sensitivity and taunt.Value == "GOD" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0,0,0 + .05 * math.cos(sine / 16)) * CFrame.Angles(math.rad(4 + 2 * math.cos(sine / 16)),math.rad(0),math.rad(-8)) * CFrame.Angles(math.rad(0),math.rad(0),math.rad(0)),1 / 3)
					Neck.C0 = Clerp(Neck.C0,nc0 * CFrame.new() * CFrame.Angles(math.rad(9 + 3 * math.sin(sine / 16)),math.rad(0),math.rad(0)),1 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.5 + .025 * math.cos(sine / 16),0) * CFrame.Angles(math.rad(8.12 + 1.5 * math.cos(sine / 16)),math.rad(-17 + 2.5 * math.sin(sine / 16)),math.rad(6.7)) * rscp,1 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5 + .03 * math.cos(sine / 16),-0) * CFrame.Angles(math.rad(0),math.rad(3.5 + 2 * math.sin(sine / 16)),math.rad(-4 + 1.5 * math.cos(sine / 16))) * lscp,1 / 3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1 - .05 * math.cos(sine / 16),-.2 + .05 * math.cos(sine / 16)) * CFrame.Angles(math.rad(-2.5 + 4 * math.cos(sine / 16)),math.rad(-12),math.rad(3)) * CFrame.Angles(math.rad(-0),math.rad(90),math.rad(0)),1 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-1 - .05 * math.cos(sine / 16),-.2 + .05 * math.cos(sine / 16)) * CFrame.Angles(math.rad(-2.5 + 4 * math.cos(sine / 16)),math.rad(15),math.rad(-5)) * CFrame.Angles(math.rad(-0),math.rad(-90),math.rad(0)),1 / 3)
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "Glitch" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0,0,3 + 1 * math.cos(sine / 24)) * CFrame.Angles(math.rad(10 + 5 * math.sin(sine / 24)),math.rad(0),math.rad(0 * math.sin(sine / 24))),.7/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.Angles(math.rad(15),math.rad(2 * math.cos(sine / 24)),math.rad(-2.5 * math.sin(sine / 24))) * CFrame.Angles(math.rad(5 * math.sin(sine / 24)),math.rad(-2 * -math.cos(sine / 24)),math.rad(2.5 * -math.sin(sine / 24))),1/3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.4 + .1 * math.cos(sine / 24),0) * CFrame.Angles(math.rad(160 + 10 * math.sin(sine / 24)),math.rad(15 + -5 * math.cos(sine / 24)),math.rad(0)) * RIGHTSHOULDERC0,.7/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.35 + .05 * math.cos(sine / 24),0) * CFrame.Angles(math.rad(-3 * math.cos(sine / 24)),math.rad(7 * math.sin(sine / 24)),math.rad(-7 + -5 * math.sin(sine / 24))) * LEFTSHOULDERC0,.7/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1 + .1 * math.cos(sine / 24),-.4) * CFrame.Angles(math.rad(-25 + 10 * math.sin(sine / 24)),math.rad(-0),math.rad(0)) * CFrame.Angles(math.rad(-0),math.rad(90),math.rad(17 * -math.sin(sine / 24))),.7/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-.7 + .07 * math.cos(sine / 24),-.5) * CFrame.Angles(math.rad(-15 + 7 * math.sin(sine / 24)),math.rad(0),math.rad(-0)) *  CFrame.Angles(math.rad(-0),math.rad(-90),math.rad(-15 * -math.sin(sine / 24))),.7/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Glitch" then
			Anim = "Walk"
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0,0,3 + 1 * math.cos(sine / 24)) * CFrame.Angles(math.rad(20-2.5*math.sin(sine/24)),math.rad(0),math.rad(0)), 1 / 3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.new() * CFrame.Angles(math.rad(-10-10*math.sin(sine/24)),math.rad(0),math.rad(0)), 1 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.4 + .1 * math.cos(sine / 24),0) * CFrame.Angles(math.rad(160 + 10 * math.sin(sine / 24)),math.rad(15 + -5 * math.cos(sine / 24)),math.rad(0)) * RIGHTSHOULDERC0, 1 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5+.25*math.sin(sine/24),0) * CFrame.Angles(math.rad(110 - 5 * math.cos(sine / 24)),math.rad(0),math.rad(0)) * CFrame.Angles(math.rad(5 * math.sin(sine / 24)),math.rad(5 * math.cos(sine / 24)),math.rad(0)) * LEFTSHOULDERC0, 1 / 3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1,-.01) * CFrame.Angles(math.rad(-10),math.rad(80),math.rad(5+5*math.sin(sine/24))), 1 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-1,-.01) * CFrame.Angles(math.rad(0),math.rad(-80),math.rad(-5-5*math.sin(sine/24))), 1 / 3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "KickGod" then
			...
			if attack == false then
				...
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)), 1 / 3)
					RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0, 0, 0 + .15 * math.cos(sine / 15)) * CFrame.Angles(math.rad(20), math.rad(0), math.rad(-0)) * CFrame.Angles(math.rad(-5 * math.cos(sine / 15)), math.rad(0), math.rad(-0)), 1 / 3)
					Neck.C0 = Clerp(Neck.C0, NECKC0 * CFrame.new(0, 0, 0 + ((1) - 1)) * CFrame.Angles(math.rad(15), math.rad(0), math.rad(0)) * CFrame.Angles(math.rad(-5 * math.sin(sine / 15)), math.rad(0), math.rad(0)), 1 / 3)
				if math.random(1,5) == 1 then
					Neck.C0 = Clerp(Neck.C0, NECKC0 * CF(0, 0, 0 + ((1) - 1)) * ANGLES(RAD(0 + MRANDOM(-25,25) - 4 * COS(Sine / 12)), RAD(MRANDOM(-25,25)), RAD(0)), 1.5 / 3)	
				end
				RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5, 0.25 + .05 * math.cos(sine / 15), 0)*CFrame.Angles(math.rad(170),math.rad(15),math.rad(0)) * CFrame.Angles(math.rad(0 + -5 * math.sin(sine / 15)),math.rad(-5 * math.cos(sine / 15)),math.rad(0))*RIGHTSHOULDERC0, 1 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0, CFrame.new(-1.2, 0.5, .5) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(50)) * CFrame.Angles(math.rad(-5 * math.sin(sine / 15)),math.rad(-5 * math.cos(sine / 15)),math.rad(0)) * LEFTSHOULDERC0, 1 / 3)
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -1.2 - .15 * math.cos(sine / 15), -.6) * CFrame.Angles(math.rad(-25), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-0), math.rad(-0), math.rad(7 * -math.cos(sine / 15))), 1 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -1 - .15 * math.cos(sine / 15), -.4) * CFrame.Angles(math.rad(25), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(-0), math.rad(0), math.rad(-7 * -math.cos(sine / 15))), 1 / 3)
			end
		elseif Torsovelocity < sensitivity and taunt.Value == "Err0r" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0 + .75 * math.cos(sine / 16) + math.sin(sine / 16),0,2.2 + .45 * math.cos(sine / 16)) * CFrame.new(0 + -.75 * -math.cos(sine / 16) + -math.sin(sine / 16),0,2.2 + -.45 * -math.sin(sine / 16)) * CFrame.Angles(math.rad(5 * math.sin(sine / 16)),math.rad(14 * math.cos(sine / 16)),math.rad(5 * math.cos(sine / 16))),.7/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.Angles(math.rad(0),math.rad(26 * math.sin(sine / 16)),math.rad(26 * math.sin(sine / 16))),.7/3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.5,0) * CFrame.Angles(math.rad(1.9),math.rad(0),math.rad(-179)) * CFrame.Angles(math.rad(1.9),math.rad(180),math.rad(16 - 10 * math.sin(sine / 16))) * RIGHTSHOULDERC0,.7/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5,0) * CFrame.Angles(math.rad(0),math.rad(0),math.rad(-10)) * CFrame.Angles(math.rad(0),math.rad(0),math.rad(10 * math.sin(sine / 16))) * LEFTSHOULDERC0,.7/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1,-.01) * CFrame.Angles(math.rad(0),math.rad(0),math.rad(10 * math.cos(sine / 16)))  * CFrame.Angles(math.rad(0),math.rad(90),math.rad(-10 * math.sin(sine / 16))),.7/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-1,-.01) * CFrame.Angles(math.rad(0),math.rad(0),math.rad(10 * math.cos(sine / 16))) *  CFrame.Angles(math.rad(0),math.rad(-90),math.rad(-10 * math.sin(sine / 16))),.7/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Err0r" then
			Anim = "Walk"
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0 + .75 * math.cos(sine / 16) + math.sin(sine / 16),0,2.5 + .45 * math.cos(sine / 16)) * CFrame.new(0 + -.75 * -math.cos(sine / 16) + -math.sin(sine / 16),0,2.5 + -.45 * -math.sin(sine / 16)) * CFrame.Angles(math.rad(40 + -3 * math.sin(sine / 16)),math.rad(-5 * math.cos(sine/16)),math.rad(5 * math.cos(sine / 16))),.7/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.new(0,-.25,0) * CFrame.Angles(math.rad(-40),math.rad(0),math.rad(0)),.7/3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.5,0) * CFrame.Angles(math.rad(-45 + -5 * math.sin(sine / 16)),math.rad(0),math.rad(5 * math.cos(sine/16))) * RIGHTSHOULDERC0,.7/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5,0) * CFrame.Angles(math.rad(-45 + -5 * math.sin(sine / 16)),math.rad(0),math.rad(-5 * math.cos(sine/16))) * LEFTSHOULDERC0,.7/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-.5,-.5) * CFrame.Angles(math.rad(-10 + -10 * math.sin(sine/16)),math.rad(80),math.rad(0)) * CFrame.Angles(math.rad(-9 * math.cos(sine/16)),math.rad(0),math.rad(0)),.7/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-1,0) * CFrame.Angles(math.rad(-10 + -10 * math.sin(sine/16)),math.rad(-80),math.rad(0)) * CFrame.Angles(math.rad(-9 * -math.cos(sine/16)),math.rad(0),math.rad(0)),.7/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "KickGod" then
			Anim = "Walk"
		...
		elseif Torsovelocity >= sensitivity and taunt.Value == "Fave" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0, 0, 0 + 0.05 * math.cos(sine / 12)) * CFrame.Angles(math.rad(0 + 3 * math.sin(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					Neck.C0 = Clerp(Neck.C0, nc0 * CFrame.new(0, 0, 0 + ((1) - 1)) * CFrame.Angles(math.rad(0 - 7 * math.cos(sine / 12)) + math.rad(0 - 3 * math.sin(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0, CFrame.new(1.5, 0.5 + 0.05 * math.cos(sine / 12), 0) * CFrame.Angles(math.rad(0), math.rad(-14 + -3 * math.cos(sine / 12)), math.rad(7 + 3 * math.sin(sine / 12))) * rscp, 0.7 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0, CFrame.new(-1.5, 0.5 + 0.05 * math.cos(sine / 12), 0) * CFrame.Angles(math.rad(0), math.rad(0), math.rad(-135)) * CFrame.Angles(math.rad(0), math.rad(-0 + -3 * math.cos(sine / 12)), math.rad(0 + -15 * math.sin(sine / 12))) * lscp, 0.7 / 3)
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -1 - 0.05 * math.cos(sine / 12), -0.01) * CFrame.Angles(math.rad(0 + 5 * math.sin(sine / 12)), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-5), math.rad(-12), math.rad(0)), 0.7 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -1 - 0.05 * math.cos(sine / 12), -0.01) * CFrame.Angles(math.rad(0 + 5 * math.sin(sine / 12)), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(-7), math.rad(15), math.rad(0)), 0.7 / 3)
				GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(0,-20,0) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "iNSaNiTY" then
			Anim = "Walk"
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0,0,3 - .5 * math.sin(sine/50)) * CFrame.Angles(math.rad(30 + 6 * math.sin(sine / 50)),math.rad(0),math.rad(0)),.7/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.Angles(math.rad(5 + 10 * math.sin(sine / 50)),math.rad(0),math.rad(0)),1)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(1.5,.5,0) * CFrame.Angles(math.rad(-41.6-4*math.cos(sine/50)),math.rad(0),math.rad(0)) * RIGHTSHOULDERC0,.7/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.5,.5,0) * CFrame.Angles(math.rad(20),math.rad(0),math.rad(-15-5*math.sin(sine/50))) * LEFTSHOULDERC0,.7/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1,-.01) * CFrame.Angles(math.rad(-20),math.rad(80),math.rad(10+10*math.cos(sine/50))),1/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-.8,-.35 + .15 * math.cos(sine / 40)) * CFrame.Angles(math.rad(-20),math.rad(-80),math.rad(-10-10*math.cos(sine/50))),1/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "Superior" then
			Anim = "Walk"
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "Sit" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0, 0, -1.8 + 0.05 * math.cos(sine / 12)) * CFrame.Angles(math.rad(5 + 3 * math.cos(sine / 12)) + math.rad(0 + 3 * math.sin(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					Neck.C0 = Clerp(Neck.C0, nc0 * CFrame.new(0, 0, 0 + ((1) - 1)) * CFrame.Angles(math.rad(5 + -10 * math.cos(sine / 12)) + math.rad(0 + -10 * math.sin(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0, CFrame.new(1.5, 0.15 + 0.05 * math.cos(sine / 12), 0) * CFrame.Angles(math.rad(-25 + 6 * math.cos(sine / 12)) + math.rad(0 + 6 * math.sin(sine / 12)), math.rad(-15), math.rad(7)) * rscp, 0.7 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0, CFrame.new(-1.5, 0.15 + 0.05 * math.cos(sine / 12), 0) * CFrame.Angles(math.rad(-25 + 6 * math.cos(sine / 12)) + math.rad(0 + 6 * math.sin(sine / 12)), math.rad(15), math.rad(-7)) * lscp, 0.7 / 3)
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -.8 - 0.05 * math.cos(sine / 12), -0.2) * CFrame.Angles(math.rad(76 + 4 * math.cos(sine / 12)) + math.rad(0 + 4 * math.sin(sine / 12)), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-4), math.rad(-15), math.rad(0)), 0.7 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -.65 - 0.05 * math.cos(sine / 12), -0.3) * CFrame.Angles(math.rad(87 + 4 * math.cos(sine / 12)) + math.rad(0 + 4 * math.sin(sine / 12)), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(-2), math.rad(17), math.rad(0)), 0.7 / 3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(-30,-30,0) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "taunt" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,ROOTC0 * CFrame.new(0,0,0 + .3 * math.cos(sine / 25)) * CFrame.new(0,0,0 + .3 * math.sin(sine / 25)) * CFrame.Angles(math.rad(-107 + 3 * math.cos(sine / 25)),math.rad(0),math.rad(0)),.1/3)
					Neck.C0 = Clerp(Neck.C0,NECKC0 * CFrame.Angles(math.rad(-18 + -5 * math.sin(sine / 25)),math.rad(0),math.rad(0)),1/3)
					RightShoulder.C0 = Clerp(RightShoulder.C0,CFrame.new(.45 + .025 * math.sin(sine / 25),.45,-.5) * CFrame.Angles(math.rad(0),math.rad(-6 + -5 * math.cos(sine / 25)),math.rad(-65 + -3 * math.sin(sine / 25))) * RIGHTSHOULDERC0,1/3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0,CFrame.new(-1.45 + -.0125 * math.sin(sine / 25),.5,0 + .025 * math.sin(sine / 25)) * CFrame.Angles(math.rad(-96 + -5 * math.cos(sine / 25)),math.rad(13),math.rad(8 + -3 * math.sin(sine / 25))) * LEFTSHOULDERC0,1/3)
					RightHip.C0 = Clerp(RightHip.C0,CFrame.new(1,-1.4,-.95) * CFrame.Angles(math.rad(-78 + -5 * math.sin(sine / 25)),math.rad(0 + -3 * math.sin(sine / 25)),math.rad(7))  * CFrame.Angles(math.rad(0),math.rad(90),math.rad(0)),1/3)
					LeftHip.C0 = Clerp(LeftHip.C0,CFrame.new(-1,-1.15,-.45) * CFrame.Angles(math.rad(-36 + -7 * math.sin(sine / 25)),math.rad(14 + 3 * math.sin(sine / 25)),math.rad(0)) *  CFrame.Angles(math.rad(0),math.rad(-90),math.rad(0)),1/3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,-1,-.15) * CFrame.Angles(math.rad(-90),math.rad(0),math.rad(0)),1/2)
			end
		elseif Torsovelocity >= sensitivity and taunt.Value == "taunt" then
			Anim = "Walk"
		...
		elseif Torsovelocity < sensitivity and taunt.Value == "taunt2" or Torsovelocity >= sensitivity and taunt.Value == "taunt2" then
			...
			if attack == false then
				...
				RootJoint.C0 = Clerp(RootJoint.C0,rc0 * CFrame.new(0, 0, 0 + 0.05 * math.cos(sine / 12)) * CFrame.Angles(math.rad(-20 + 3 * math.cos(sine / 12)) + math.rad(0 + 5 * math.sin(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					Neck.C0 = Clerp(Neck.C0, nc0 * CFrame.new(0, 0, 0 + ((1) - 1)) * CFrame.Angles(math.rad(5 + 5 * math.cos(sine / 12)), math.rad(0), math.rad(0)), 0.7 / 3)
					RightShoulder.C0 = Clerp(RightShoulder.C0, CFrame.new(.6, 0.35 + 0.05 * math.cos(sine / 12), -.5) * CFrame.Angles(math.rad(0 + 5 * math.cos(sine / 12)), math.rad(5 + 3 * math.cos(sine / 12)) + math.rad(0 + -3 * math.sin(sine / 12)), math.rad(-90)) * rscp, 0.7 / 3)
					LeftShoulder.C0 = Clerp(LeftShoulder.C0, CFrame.new(-.6, 0.2 + 0.05 * math.cos(sine / 12), -.6) * CFrame.Angles(math.rad(0 + 5 * math.cos(sine / 12)), math.rad(-5 + -3 * math.cos(sine / 12)) + math.rad(0 + -3 * math.sin(sine / 12)), math.rad(90)) * lscp, 0.7 / 3)
					RightHip.C0 = Clerp(RightHip.C0, CFrame.new(1, -1 - 0.05 * math.cos(sine / 12), -0.25) * CFrame.Angles(math.rad(-20 + 5 * math.cos(sine / 12)) + math.rad(0 + 7 * math.sin(sine / 12)), math.rad(90), math.rad(0)) * CFrame.Angles(math.rad(-3), math.rad(-12), math.rad(0)), 0.7 / 3)
					LeftHip.C0 = Clerp(LeftHip.C0, CFrame.new(-1, -1 - 0.05 * math.cos(sine / 12), -0.25) * CFrame.Angles(math.rad(-20 + 5 * math.cos(sine / 12)) + math.rad(0 + 7 * math.sin(sine / 12)), math.rad(-90), math.rad(0)) * CFrame.Angles(math.rad(-5), math.rad(15), math.rad(0)), 0.7 / 3)
					GunJoint.C0 = Clerp(GunJoint.C0,CFrame.new(.05,35,-.15) * CFrame.Angles(math.rad(90),math.rad(0),math.rad(0)),1/2)
			end
		end
	end
